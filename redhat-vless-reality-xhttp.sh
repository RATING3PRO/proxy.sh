...
install_deps() {
  ...
  if command -v dnf >/dev/null; then
    dnf install -y curl wget unzip jq openssl ca-certificates
  else
    yum install -y curl wget unzip jq openssl ca-certificates
  fi
  
  # 尝试安装 tar，有些精简版系统可能缺失
  if ! command -v tar >/dev/null; then
     if command -v dnf >/dev/null; then dnf install -y tar; else yum install -y tar; fi
  fi
}
